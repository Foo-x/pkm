...
また、配列を昇順にソートする関数のプロパティベーステストの例は以下のようになる。
...
- 複数の操作を違う順序で実行しても同じ出力になること
    - 可換性のある操作があるときに有効で、同時にテストできる
    - 配列をmapしてからソート、ソートしてからmap
        - `arr.map(x => x + 1).sorted() == arr.sorted().map(x => x + 1)`
- 信頼できる実装と同じ出力になること
    - 内部処理を無視すれば単純な出力のときに有効
        - 既存実装のパフォーマンス改善版など
    - 独自実装のmaxと標準のmax
        - `myMax(arr) == Math.max(arr)`
- 操作をしても変わらないこと（不変条件）
    - 配列をmapしたときの配列の長さ
...
| 観点 | プロパティベーステスト | 事例ベーステスト |
| --- | --- | --- |
| メリット | ・エッジケースを見つけやすい<br>・少ないコードで広いカバレッジを実現できる<br>・プロパティを考える過程で仕様漏れに気づける | ・値が明確なのでわかりやすく、使用例としてのドキュメントにもなる<br>・固定の値のみテストするので高速<br>・学習コストが低い<br>・特定の入出力ペアをリグレッションテストとして固定できる |
| デメリット | ・学習コストが高い<br>・大量の値でテストするので遅い<br>・ランダム性があるので脆いテストになりやすい | ・エッジケースを見落としやすい<br>・網羅性を上げると似たようなテストが大量にできる |
| 使いどころ | ・プロパティとして表現しやすい数学的な性質があるとき<br>・不変条件があるとき<br>・エッジケースの漏れを見つけたいとき<br>・入力の範囲が広く、事例ベースだと網羅が難しいとき | ・特定の入出力ペアが仕様として定まっているとき<br>・使用例としてのドキュメントにしたいとき<br>・複雑な処理でプロパティとして表現しにくいとき<br>・入力の範囲が狭く、総当たりでテストできるとき |
